...
async function rpc(method,params,{timeoutMs=35000,signal}={}){const {base,token}=await accessToken();const id=++nextId;const deadline=AbortSignal.timeout(timeoutMs);const r=await fetch(base+'/mcp',{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json',Accept:'application/json, text/event-stream'},body:JSON.stringify({jsonrpc:'2.0',id,method,params}),signal:signal?AbortSignal.any([signal,deadline]):deadline,redirect:'error'});if(r.status===401||r.status===403)throw new AuthError('Access denied. Reconnect from login or Settings.');if(!r.ok)throw new Error(`MCP request failed (${r.status})`);
