...
async function watch(){
  ...
  try{
    ...
    await observe({sessionId,events,continuous:flag('continuous'),signal:abort.signal,
      ...
      deliver:async event=>{if(abort.signal.aborted)return; if(delivery==='orca')await sendOrca(origin,event,{signal:abort.signal});else if(delivery==='bridge')await deliverBridge(option('socket'),thread,feedbackMessage(event));else if(delivery==='http')await deliverHttp(adapter,thread,event);else if(delivery==='codex')await queueEvent(thread,event,remote);else await new Promise((resolve,reject)=>process.stdout.write(JSON.stringify(event)+'\n',e=>e?reject(e):resolve()));await write(statusPath,{...status,lastEventAt:new Date().toISOString(),lastEventId:event.event_id})},
...
try{
  ...
  switch(command){
    ...
    case 'describe':{const agent=agents.find(a=>a.id===args[0]);if(!agent)throw new Error('Unknown host');console.log(JSON.stringify({...agent,transport:'websocket-hibernation',reference:'references/agents.md'},null,2));break;}
