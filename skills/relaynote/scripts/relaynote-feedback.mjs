...
try{
  ...
  switch(command){
    ...
    case 'describe':{
      ...
      console.log(JSON.stringify({...agent,transport:'websocket-hibernation',sharedRuntime:{protocol:1,reference:'references/monitoring.md',setup:'relaynote-runtime.mjs setup --accept-install',registration:'Existing conversation only; verify this host recipe first'},deliveryProtocol:3,eventScope:'submitted-discussions-and-final-decisions',events:{discussions:{default:true,serverCapability:'discussion_protocol: 1',fallback:'final-decisions only when the server does not advertise it',optOut:'--events decisions',reference:'references/receiving.md'}},artifacts:{reference:'references/artifacts.md',requires:['Node.js >=22.13','Relaynote renderer checkout with pinned npm dependencies','local Google Chrome']},reference:'references/monitoring.md'},null,2));break;
    }
