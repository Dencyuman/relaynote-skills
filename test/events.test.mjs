...
test('idle wait deadlines never fetch data; only ready or changed events do',async()=>{
 ...
 const originalFetch=globalThis.fetch,originalSocket=globalThis.WebSocket;let reads=0,ws;const tickets=[];
 globalThis.fetch=async (url,options)=>{if(String(url).endsWith('/events-ticket'))tickets.push(options.headers);if(String(url).endsWith('/snapshot')){reads++;return Response.json({session_id:'session',updated_at:'t'+reads,current_round:1})}return Response.json({ticket:'test'})};
 ...
 const {eventSource}=await import('../skills/relaynote/scripts/lib/events.mjs');const source=eventSource('session',{bindingId:'this-conversation'});
 try{
  await source.ready();assert.equal(tickets[0]['X-Relaynote-Binding'],'this-conversation');await source.wait('session','t0',0.001);assert.equal(reads,1);
